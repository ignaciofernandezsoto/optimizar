    ...
    stages {
        stage('Create new container name') {
            steps {
                script {
                    ...
                    def new_full_container_name = sh script:'echo "${OPTIMIZAR_FULL_IMAGE_NAME}_' + trimmed_current_date + '_' + trimmed_random_string + '"', returnStdout: true
        ...
        stage('Stop and delete previous docker container') {
            steps {
                sh 'docker ps -a \
                  | awk \'{ print $1,$2 }\' \
                  | grep ${OPTIMIZAR_SHORT_IMAGE_NAME} \
                  | awk \'{print $1 }\' \
                  | xargs -I {} docker rm -f {}'
              }
        }
        stage('Run new docker container') {
            steps {
                sh 'docker run --network="host" --name ${OPTIMIZAR_SHORT_IMAGE_NAME} -d --restart unless-stopped ' + NEW_FULL_CONTAINER_NAME
            }
        }
    }
}